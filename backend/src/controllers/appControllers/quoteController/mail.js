...
const mail = async (req, res) => {
  ...
  try {
    ...
    const pdfFileName = `quote-${id}.pdf`;
    console.log('PDF File Name:', pdfFileName);

    const pdfFilePath = path.join('D:/idurar-erp-crm-master/idurar-erp-crm-master/backend/src/', 'public', 'download', 'quote', pdfFileName); // Adjust the path according to your project structure
    ...
    const emailResults = await Promise.all(
      emailAddresses.map(email => sendMail(email, "Your Quote from ProManage", message, pdfFilePath))
    );
