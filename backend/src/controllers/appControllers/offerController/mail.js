...
const mail = async (req, res) => {
  ...
  try {
    ...
    const pdfFileName = `offer-${id}.pdf`;
    console.log('PDF File Name:', pdfFileName);

    const pdfFilePath = path.join('D:/idurar-erp-crm-master/idurar-erp-crm-master/backend/src/', 'public', 'download', 'offer', pdfFileName); // Adjust the path according to your project structure
    ...
    const emailResults = await Promise.all(
      emailAddresses.map(email => sendMail(email, "Your Offer from ProManage", message, pdfFilePath))
    );
